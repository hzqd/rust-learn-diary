...
impl Solution {
    pub fn generate_matrix(n: i32) -> Vec<Vec<i32>> {
        ...
        let mut i = 1;
        loop {
            for y in y_min..y_max { res[x_min][y] = i; i+=1; }
            x_min += 1;
            if x_min == x_max { break }
            for x in x_min..x_max { res[x][y_max-1] = i; i+=1; }
            y_max -= 1;
            if y_min == y_max { break }
            for y in (y_min..y_max).rev() { res[x_max-1][y] = i; i+=1; }
            x_max -= 1;
            if x_min == x_max { break }
            for x in (x_min..x_max).rev() { res[x][y_min] = i; i+=1; }
            y_min += 1;
            if y_min == y_max { break }
        }
        res
